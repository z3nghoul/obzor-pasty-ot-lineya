...
struct clr_t {
    ...
    __forceinline clr_t( ) = default;

    __forceinline clr_t( float r, float g, float b ) {
        m_r = r;
        m_g = g;
        m_b = b;
    }
    ...
    __forceinline void increment( bool bigger_inc = false ) {
        if ( bigger_inc ) {
            m_r += 70.f;
            m_g += 70.f;
            m_b += 70.f;
    ...
    __forceinline void decrease( bool bigger_dec = false ) {
    ...
    __forceinline void clamp( ) {
        m_r = std::clamp( m_r, 40.f, 55.f );
        m_g = std::clamp( m_g, 41.f, 56.f );
        m_b = std::clamp( m_b, 45.f, 60.f );
    }
    ...
    __forceinline int g( ) {
        return static_cast < int > ( m_g );
    }

    __forceinline int b( ) {
        return static_cast < int > ( m_b );
    }
};
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    static std::map < ImGuiID, clr_t > clr_modulation{};
    static std::map < ImGuiID, clr_t > clr_top_modulation{};

    auto cur_element = clr_top_modulation.find( id );

    if ( cur_element == clr_top_modulation.end( ) ) {
        clr_top_modulation.insert( { id, clr_t( 41.f, 42.f, 46.f ) } );
        cur_element = clr_top_modulation.find( id );
    }

    if ( pressed ) {
        cur_element->second.increment( true );
    }
    else {
        cur_element->second.decrease( true );
    }

    cur_element->second.m_r = std::clamp( cur_element->second.m_r, 41.f, 60.f );
    cur_element->second.m_g = std::clamp( cur_element->second.m_g, 42.f, 61.f );
    cur_element->second.m_b = std::clamp( cur_element->second.m_b, 46.f, 65.f );

    /* nigger */
    auto cur_border_element = clr_modulation.find( id );

    if ( cur_border_element == clr_modulation.end( ) ) {
        clr_modulation.insert( { id, clr_t( 40.f, 41.f, 45.f ) } );
        cur_border_element = clr_modulation.find( id );
    }

    if ( hovered ) {
        cur_border_element->second.increment( );
    }
    else {
        cur_border_element->second.decrease( );
    }

    cur_border_element->second.clamp( );

    ImColor clr = ImColor( cur_border_element->second.r( ), cur_border_element->second.g( ), cur_border_element->second.b( ), static_cast < int > ( style.Alpha * 255.f ) );

    if ( cur_element->second.m_r != 41.f
        && cur_element->second.m_g != 42.f
        && cur_element->second.m_b != 46.f )
        clr = ImColor( cur_element->second.r( ), cur_element->second.g( ), cur_element->second.b( ), static_cast < int > ( style.Alpha * 255.f ) );

    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, ImColor ( 27, 28, 32, static_cast < int > ( style.Alpha * 255.f ) ), true, 1.f );
    window->DrawList->AddRect( bb.Min - ImVec2( 1, 1 ), bb.Max + ImVec2( 1, 1 ), clr, 1.f );
    RenderTextClipped(bb.Min + style.FramePadding - ImVec2 ( 0, 2 ), bb.Max - style.FramePadding + ImVec2( 0, 2 ), label, NULL, &label_size, style.ButtonTextAlign, &bb );
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float deltatime = 2.f * ImGui::GetIO( ).DeltaTime;
    ...
    if ( *v )
        it_hover->second = std::clamp( it_hover->second + ( 3.f * ImGui::GetIO( ).DeltaTime * ( hovered ? 1.f : -1.f ) ), 1.0f, 1.f );
    else
        it_hover->second = std::clamp( it_hover->second + ( 3.f * ImGui::GetIO( ).DeltaTime ), 1.0f, 1.f );

    const ImVec4 hover_dis = ImColor( 154, 171, 199, 255 );
    const ImVec4 hover_act = ImColor( 27, 28, 32, 255 );
    ...
    static std::map < ImGuiID, float > checkmark_alpha{};

    auto cur_element = checkmark_alpha.find( id );

    if ( cur_element == checkmark_alpha.end( ) )
    {
        checkmark_alpha.insert( { id, 0.f } );
        cur_element = checkmark_alpha.find( id );
    }

    if ( *v )
        cur_element->second += 10.f;
    else
        cur_element->second -= 10.f;

    const auto rounded_alpha = static_cast < int > ( style.Alpha * 255.f );

    if ( *v ) {
        if ( rounded_alpha != 255
            && rounded_alpha != 0.f )
            cur_element->second = rounded_alpha;
    }

    cur_element->second = std::clamp( cur_element->second, 0.f, 255.f );
    ...
    auto cur__border_element = clr_modulation.find( id );

    if ( cur__border_element == clr_modulation.end( ) ) {
        clr_modulation.insert( { id, clr_t( 40.f, 41.f, 45.f ) } );
        cur__border_element = clr_modulation.find( id );
    }

    if ( hovered ) {
        cur__border_element->second.increment( );
    }
    else {
        cur__border_element->second.decrease( );
    }

    cur__border_element->second.clamp( );

    ImColor clr = ImColor( cur__border_element->second.r( ), cur__border_element->second.g( ), cur__border_element->second.b( ), static_cast < int > ( style.Alpha * 255.f ) );

    RenderFrame( check_bb.Min - ImVec2( 1, 1 ), check_bb.Max + ImVec2( 1, 1 ), GetColorU32( ImVec4( it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second ) ), true, 1.f );

    const float pad = ImMax( 1.0f, IM_FLOOR( GetFrameHeight( ) / 4.0f ) );
    RenderCheckMark( window->DrawList, check_bb.Min, ImColor( 255, 255, 255, static_cast < int > ( cur_element->second ) ), GetFrameHeight( ) - pad * 2.0f );

    ImGui::PushStyleColor( ImGuiCol_Text, it_text->second );
    if ( label_size.x > 0.0f )
        RenderText( text_bb.GetTL( ) + ImVec2( 8, 0 ) - ImVec2( 0, 1 ), label );

    window->DrawList->AddRect( check_bb.Min - ImVec2( 2, 2 ), check_bb.Max + ImVec2( 2, 2 ), clr, 1.f, ImDrawCornerFlags_All, window->WindowBorderSize );
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool animate )
{
    ImGuiContext& g = *GImGui;
    ...
    static std::map<const ImGuiID, clr_t > clr_modulation{};
    static std::map < const ImGuiID, float > height_for_anim{};

    auto height_element = height_for_anim.find( id );

    if ( height_element == height_for_anim.end( ) ) {
        height_for_anim.insert( { id, -80.f } );
        height_element = height_for_anim.find( id );
    }

    auto cur_element = clr_modulation.find( id );

    if ( cur_element == clr_modulation.end( ) ) {
        clr_modulation.insert( { id, clr_t( 40.f, 41.f, 45.f ) } );
        cur_element = clr_modulation.find( id );
    }

    if ( hovered || popup_open ) {
        cur_element->second.increment( );
    }
    else {
        cur_element->second.decrease( );
    }

    cur_element->second.clamp( );


    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
    RenderNavHighlight(frame_bb, id);
    if ( !( flags & ImGuiComboFlags_NoPreview ) ) {
        ImColor clr = ImColor( cur_element->second.r( ), cur_element->second.g( ), cur_element->second.b( ), 255 );

        window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( value_x2, frame_bb.Max.y ), ImColor( 27, 28, 32, 255 ), 1.f, ( flags & ImGuiComboFlags_NoArrowButton ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left ); // main rect gde render elementov
        window->DrawList->AddRect( frame_bb.Min, ImVec2( value_x2, frame_bb.Max.y ), clr, 1.f );
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y - 20.f, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f); // arrow down/up 
    }
    RenderFrameBorder(frame_bb.Min, frame_bb.Max, 1.f);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f)); // render choosen item-a
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 20.f, frame_bb.Min.y + style.FramePadding.y), label); // text tipo no ego sdelat sverhu
    ...
    if ( !popup_open ) {
        height_element->second = -40.f;
        return false;
    }
    height_element->second += 1.3f;

    height_element->second = std::clamp( height_element->second, -40.f, 0.f );
    ...
    bool ret = Begin(name, true, NULL, window_flags, height_element->second );
    PopStyleVar();
    if (!ret)
    {
        EndPopup();
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items, bool animate )
{
    ImGuiContext& g = *GImGui;
    ...
    static std::map < const ImGuiID, float > anim{};

    auto cur_element = anim.find( id );

    if ( cur_element == anim.end( ) ) {
        anim.insert( { id, -27.f } );
        cur_element = anim.find( id );
    }

    if ( !BeginCombo( label, preview_value, ImGuiComboFlags_None, animate ) ) {
        cur_element->second = -27.f;
        return false;
    }

    cur_element->second += 1.f;

    cur_element->second = std::clamp( cur_element->second, -27.f, 0.f );
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if ( Selectable( item_text, item_selected, 0, ImVec2( ), cur_element->second ) )
        {
            value_changed = true;
            *current_item = i;
        }
...
bool ImGui::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items, bool animate )
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, animate );
    return value_changed;
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImColor clr = ImColor( cur_element->second.r( ), cur_element->second.g( ), cur_element->second.b( ), static_cast < int > ( style.Alpha * 255 ) );

    window->DrawList->AddRectFilled( clickable.Min + ImVec2( 0.f, 7.f ), clickable.Max - ImVec2( 0.f, 7.f ), ImColor( 27, 28, 32, static_cast < int > ( style.Alpha * 255 ) ), 1.f );

    window->DrawList->AddRectFilled( clickable.Min + ImVec2( 0.f, 7.f ), grab_bb.Max - ImVec2( 10.f, 5.f ), ImColor( 154, 171, 199, static_cast < int > ( style.Alpha * 255 ) ), 1.f );

    window->DrawList->AddRect( clickable.Min + ImVec2( 0.f, 6.f ), clickable.Max - ImVec2( 0.f, 6.f ), clr, 1.f, ImDrawCornerFlags_All, window->WindowBorderSize );

    window->DrawList->AddCircleFilled( grab_bb.Min + ImVec2( 5.f, 7.f ), 6.f + it_anim->second.circle_anim, ImColor( 255, 255, 255, static_cast < int > ( style.Alpha * 255 ) ), 32.f );
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    flags |= ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_AlphaPreviewHalf;
    ...
    const auto label_size = CalcTextSize( label, label_display_end, true );
    const ImVec2 pos = window->DC.CursorPos;
    ...
    {
        open_animation.insert( { id, {false, 0.f} } );
        it_open = open_animation.find( id );
    }
    it_open->second.mult = ImClamp( it_open->second.mult + ( 6.f * GetIO( ).DeltaTime * ( it_open->second.play ? 1.f : -1.f ) ), 0.01f, 1.f );
    ...
    it_active->second = ImClamp( it_active->second + ( 2.f * ImGui::GetIO( ).DeltaTime ), 0.0f, 1.0f );
    it_active->second *= style.Alpha;

   // ImGui::PushFont( InterMediumone );
    {
        if ( label != label_display_end && !( flags & ImGuiColorEditFlags_NoLabel ) )
        {
            const auto cursorpos = window->DC.CursorPos;
            window->DC.CursorPos += ImVec2( 24.f, -1 );
        ...
        if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) )
        {
            ...
            if ( BeginPopup( "picker", ImGuiWindowFlags_NoMove ) )
            {
                ...
                PushItemWidth( std::min( it_open->second.mult, 1.f ) * square_sz * 10.0f ); // Use 256 + bar sizes?
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) // тут менять колорпикер крч пизда я бот
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (alpha_bar)
    {
        ...
        InvisibleButton("alpha", ImVec2(bars_width, sv_picker_size - 4.f ));
        if (IsItemActive())
        {
    ...
    sv_cursor_rad = std::clamp( sv_cursor_rad, 6.f, 10.f );

    draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 32);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad + 1, col_midgrey, 32);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad, col_white, 32);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    ImColor clr = ImColor( cur_border_element->second.r( ), cur_border_element->second.g( ), cur_border_element->second.b( ), 255 );
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb_without_alpha), 1.f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 1.f );
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 1.f, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            RenderFrameBorder(bb.Min, bb.Max, 1.f );
        else
            window->DrawList->AddRect(bb.Min, bb.Max, clr, 1.f ); // Color button are often in need of some sort of border
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, float ext )
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (held && (flags & ImGuiSelectableFlags_DrawHoveredWhenHeld))
        hovered = true;
    if (hovered || selected) // blue zalupa
    {
        //const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
        //RenderFrame(bb.Min, bb.Max - ImVec2 ( 430.f, 0.f ), col, false, 0.0f );
    ...
    RenderTextClipped(text_min - ImVec2 ( 0.f, 5.f ) + ImVec2 ( 0, ext ), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb );
...
bool ImGui::Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, float ext )
{
    if (Selectable(label, *p_selected, flags, size_arg, ext ))
    {
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = Begin("##MainMenuBar", false, NULL, window_flags) && BeginMenuBar();
    PopStyleVar(2);
